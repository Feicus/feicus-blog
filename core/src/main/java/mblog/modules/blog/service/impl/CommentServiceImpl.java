...
public class CommentServiceImpl implements CommentService {
	...
	@Override
	public Page<CommentVO> paging4Home(Pageable pageable, long authorId,int isDelete) {
		Page<Comment> page = commentDao.findAllByAuthorIdAndIsDeleteOrderByCreatedDesc(pageable, authorId,isDelete);
    ...
    @Override
    public Page<CommentVO> paging(Pageable pageable, long toId,int isDelete) {
        Page<Comment> page = commentDao.findAllByToIdAndIsDeleteOrderByCreatedDesc(pageable, toId,isDelete);
    ...
    @Override
    @Transactional
    public void updateDelete(long id, long authorId,int isDelete){
        Comment po = commentDao.findOne(id);
        if (po != null) {
            // 判断文章是否属于当前登录用户
            Assert.isTrue(po.getAuthorId() == authorId, "认证失败");
            int status=Consts.DELETE_ACTIVE==isDelete?Consts.DELETE_ACTIVE:Consts.DELETE_DEFAULT;
