...
public interface PostDao extends JpaRepository<Post, Long>, JpaSpecificationExecutor<Post>, PostDaoCustom {
    ...
    Page<Post> findAllByAuthorIdAndIsDeleteOrderByCreatedDesc(Pageable pageable, long authorId,int isDelete);


	// findLatests
	List<Post> findTop10ByOrderByCreatedDesc();

	List<Post> findTop10ByIsDeleteOrderByCreatedDesc(int isDelete);

	// findHots
	List<Post> findTop10ByOrderByViewsDesc();
	List<Post> findTop10ByIsDeleteOrderByViewsDesc(int isDelete);


	List<Post> findAllByIdIn(Collection<Long> id);

    List<Post> findAllByIdInAndIsDelete(Collection<Long> id,int isDelete);
	...
	Post findByIdAndIsDelete(long id,int isDelete);
}
