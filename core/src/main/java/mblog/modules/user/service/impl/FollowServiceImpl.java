...
public class FollowServiceImpl implements FollowService {
	...
	public long follow(long userId, long followId) {
		...
		Follow f = followDao.findByUserAndFollowAndIsDelete(new User(userId), new User(followId),Consts.DELETE_DEFAULT);
		Assert.isNull(f,"已经关注,不能重复关注");
	...
	public void unfollow(long userId, long followId) {
		Follow po = followDao.findByUserAndFollowAndIsDelete(new User(userId), new User(followId),Consts.DELETE_ACTIVE);
		Assert.isNull(po,"已经取消关注,不能重复取消");
		Follow p = followDao.findByUserAndFollow(new User(userId), new User(followId));
		Assert.notNull(p,"暂未关注此用户");
		p.setIsDelete(Consts.DELETE_ACTIVE);
		followDao.save(p);
	...
	public Page<UserVO> follows(Pageable pageable, long userId) {
		Page<Follow> page = followDao.findAllByUserAndIsDelete(pageable, new User(userId),Consts.DELETE_DEFAULT);
	...
	public Page<UserVO> fans(Pageable pageable, long userId) {
		Page<Follow> page = followDao.findAllByFollowAndIsDelete(pageable, new User(userId),Consts.DELETE_DEFAULT);
		List<UserVO> rets = new ArrayList<>();
	...
	public boolean checkFollow(long userId, long followId) {
		return (followDao.findByUserAndFollowAndIsDelete(new User(userId), new User(followId),Consts.DELETE_DEFAULT) != null);
	}
