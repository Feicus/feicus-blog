...
public class PostController extends BaseController {
    ...
    @RequiresPermissions("post:logicdelete")
	public @ResponseBody Data logicdelete(@RequestParam("id") Long id) {
		Data data = Data.failure("操作失败");
		if (id != null) {
			try {
				postService.updateDelete(id,Consts.DELETE_ACTIVE);
				data = Data.success("操作成功", Data.NOOP);
			} catch (Exception e) {
				data = Data.failure(e.getMessage());
			}
    ...
    @RequiresPermissions("post:recovery")
	public @ResponseBody Data recovery(@RequestParam("id") Long id) {
		Data data = Data.failure("操作失败");
		if (id != null) {
			try {
				postService.updateDelete(id,Consts.DELETE_DEFAULT);
				data = Data.success("操作成功", Data.NOOP);
			} catch (Exception e) {
				data = Data.failure(e.getMessage());
			}
