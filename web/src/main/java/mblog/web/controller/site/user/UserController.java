...
public class UserController extends BaseController {
	...
	public String home(ModelMap model) {
		...
		Page<FeedsVO> page = feedsService.findUserFeeds(pageable, subject.getProfile().getId(),Consts.DELETE_DEFAULT);
	...
	public String posts(ModelMap model) {
		...
		Page<PostVO> page = postService.pagingByAuthorId(pageable, up.getId(),Consts.DELETE_DEFAULT);
	...
	public String comments(ModelMap model) {
		Pageable pageable = wrapPageable();
		AccountSubject subject = getSubject();
		Page<CommentVO> page = commentService.paging4Home(pageable, subject.getProfile().getId(),Consts.DELETE_DEFAULT);
		model.put("page", page);
		initUser(model);
	...
	public String favors(ModelMap model) {
		...
		Page<FavorVO> page = favorService.pagingByOwnId(pageable, profile.getId(),Consts.DELETE_DEFAULT);
